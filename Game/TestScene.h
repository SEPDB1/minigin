...
namespace dae
{
	namespace test
	{
		static void LoadTestScene()
		{
			...
			//playerInputLogo.AddCommandBinding("Move", std::make_unique<TankMoveCommand>(std::addressof(logo), 2.f));
			...
			//playerInputTitle.AddCommandBinding("Move", std::make_unique<TankMoveCommand>(std::addressof(title), 4.f));
